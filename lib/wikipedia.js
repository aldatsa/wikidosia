...
var Wikipedia = function() {
    ...
    // https://twitter.com/euwikipedia/status/939450309981679618
    this.false_positives_eu = [
        "Taterillus_gracilis",
        "Gaston_Julia"
    ];
};

Wikipedia.prototype.getSearchResults = function(language_code, query) {

    var promise = new Promise(function(resolve, reject) {

        request({
            url: "http://" + language_code + ".wikipedia.org/w/api.php?action=query&list=search&format=json&srsearch=" + query,
            json: true
                ...
                if (body.items[0].articles.length > 0) {
                    ...
                    for (var i = 0; results.length < how_many; i++) {
                        ...
                        if (!exclude) {
                            ...
                            if (!that.false_positives_eu.includes(body.items[0].articles[i].article)) {
