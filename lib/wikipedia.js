...
Wikipedia.prototype.getDailyPageViews = function(language_code, article, start_year, start_month, start_day, end_year, end_month, end_day, access, agent) {
    ...
    var promise = new Promise(function(resolve, reject) {
        ...
        console.log("https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/" + language_code + ".wikipedia/" + access + "/" + agent + "/" + article + "/daily/" + start_year + start_month + start_day + "/" + end_year + end_month + end_day);

        request({
            url: "https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/" + language_code + ".wikipedia/" + access + "/" + agent + "/" + article + "/daily/" + start_year + start_month + start_day + "/" + end_year + end_month + end_day,
            json: true
        }, function (error, response, body) {
