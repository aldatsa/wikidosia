...
var Wikidosia = function (config) {
    ...
    // Ausazko artikuluak.
    // EDT ordua (-6 ordu)
    // Momentuz asteartetik igandera 21:00tan. Argi ordu aldaketekin!
    var nabarmendutakoak_txioak = crontab.scheduleJob(config.schedule_job_nabarmenduak, function() {
        ...
        self.eskuratuHurrengoNabarmendua().then(function(emaitza) {
            ...
            self.txiokatuNabarmendua(emaitza, kategoria);
            self.tootNabarmendua(emaitza, kategoria);
    ...
    var asteko_irudiaren_txioak = crontab.scheduleJob(config.schedule_job_picture_of_the_week, function() {
        ...
        self.tootAstekoIrudia();

    });

    // Asteko herriaren txioak.
    // 2019an asteko herririk ez dagoela dirudi: https://eu.wikipedia.org/wiki/Wikiproiektu:Euskal_Herriko_udalerriak/Asteko_herria
    // EDT ordua (-6 ordu))
...
Wikidosia.prototype.txiokatuNabarmendua = function(nabarmendua, kategoria) {
    ...
    if (kategoria !== "") {
        ...
        mezua = kategoria + " kategoriako #ArtikuluNabarmendua: " + nabarmendua.izena + " " + nabarmendua.esteka;

    } else {

        mezua = "Gaurko #ArtikuluNabarmendua: " + nabarmendua.izena + " " + nabarmendua.esteka;

    }

    euskalwikipedia.eskuratuArtikuluarenIrudia(nabarmendua.esteka).then(function(irudiak) {
...
Wikidosia.prototype.tootNabarmendua = function(nabarmendua, kategoria) {
    ...
    euskalwikipedia.eskuratuArtikuluarenIrudia(nabarmendua.esteka).then(function(irudiak) {
        ...
        // Irudirik ez badago, hau da, irudiak {} bada...
        //
        if (Object.keys(irudiak).length === 0) {
        ...
        // Irudia badago...
            ...
            var fitxategia = path.basename(url.parse(irudiak.x2).pathname);

            self.deskargatu(irudiak.x2, "./openshift/tmp/" + fitxategia).then(function() {
...
Wikidosia.prototype.txiokatuAstekoIrudia = function() {
    ...
    euskalwikipedia.eskuratuAstekoIrudia().then(function(asteko_argazkia) {
        ...
        image_request.get(asteko_argazkia.irudiak.x1, function (error, response, body) {
            ...
            if (!error && response.statusCode == 200) {
                ...
                T.post('media/upload', { media_data: b64content }, function (err, data, response) {
                    ...
                    mezua = mezua + " " + asteko_argazkia.irudiak.fitxategi_orria + " #AstekoIrudia";
...
Wikidosia.prototype.tootAstekoIrudia = function() {
    ...
    euskalwikipedia.eskuratuAstekoIrudia().then(function(asteko_argazkia) {
        ...
        //https://stackoverflow.com/a/27006391/2855012
        var fitxategia = path.basename(url.parse(asteko_argazkia.irudiak.x1).pathname);

        self.deskargatu(asteko_argazkia.irudiak.x1, "./openshift/tmp/" + fitxategia).then(function() {
