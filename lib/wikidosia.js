    ...
    telegram_bot.on('inline_query', function (query) {
        ...
        wikipedia.getSearchResults("eu", query.query).then(function(results) {
            if (results.query && results.query.search) {
                results = results.query.search.map(function (elem) {
                    return {
                        id: elem.title,
                        type: "article",
                        title: elem.title,
                        input_message_content: "input_message_content zer da hau?",
                        url: "https://eu.wikipedia.org/wiki/" + elem.title.replace(/ /g, '_')
                    };
                });
            }
        });

        telegram_bot.answerInlineQuery(query.id, results);
    });
        ...
        image_request.get(asteko_argazkia.irudiak.x2, function (error, response, body) {
            ...
            if (!error && response.statusCode == 200) {
                ...
                T.post('media/upload', { media_data: b64content }, function (err, data, response) {
                    ...
                    var mezua = asteko_argazkia.testua.substring(0, 140 - 65);
