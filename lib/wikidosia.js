...
var Wikidosia = function (config) {
    ...
    // EDT ordua (-6 ordu))
    // Momentuz astearteetan 15:00tan. Argi ordu aldaketekin!
...
Wikidosia.prototype.txiokatuNabarmendua = function() {
    ...
    this.eskuratuHurrengoNabarmendua().then(function(emaitza) {

        if (emaitza["azpi-kategoria"] !== "") {

            mezua = emaitza["azpi-kategoria"] + " kategoriako #ArtikuluNabarmendua: " + emaitza.izena + " " + emaitza.esteka;
        ...
        if (!error && response.statusCode == 200) {
            ...
            request({
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    var $ = cheerio.load(body);
                    var srcset = "http:" + $("#mw-content-text .infotaula a.image img").attr("srcset");
                    srcset = srcset.split(" ");

                    herria.irudiak.x1 = $("#mw-content-text .infotaula a.image img").attr("src");
                    herria.irudiak.x1_5 = srcset[0];
                    herria.irudiak.x2 = srcset[2];

                    console.log(herria.irudiak);

                    var image_request = require('request').defaults({ encoding: null });

                    image_request.get("https:" + herria.irudiak.x2, function (error, response, body) {
                    ...
                    console.log("Errorea gertatu da asteko herriaren irudiaren datuak eskuratzean.");
                    console.log(error);
